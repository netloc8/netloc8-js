...
function collectNavigationTiming(): void {
    ...
    try {
        ...
        if (nav.domainLookupEnd > 0 && nav.domainLookupStart > 0) {
            collectedMetrics.dns = Math.round(nav.domainLookupEnd - nav.domainLookupStart);
        }
        if (nav.secureConnectionStart > 0 && nav.connectEnd > 0) {
            collectedMetrics.tls = Math.round(nav.connectEnd - nav.secureConnectionStart);
        }
        if (nav.responseStart > 0 && nav.requestStart > 0) {
            collectedMetrics.request = Math.round(nav.responseStart - nav.requestStart);
        }
        if (nav.responseEnd > 0 && nav.responseStart > 0) {
            collectedMetrics.response = Math.round(nav.responseEnd - nav.responseStart);
        }
    } catch {
...
function sendBeacon(endpoint: string): void {
    // Only send if we have metrics or errors
    if (Object.keys(collectedMetrics).length === 0 && collectedErrors.length === 0) {
        return;
    }
    ...
    if (Object.keys(collectedMetrics).length > 0) {
        beacon.metrics = { ...collectedMetrics };
    }

    if (collectedErrors.length > 0) {
        beacon.errors = collectedErrors.slice(0, MAX_ERRORS);
    }
...
export function initRum(config?: RumConfig): () => void {
    ...
    import('web-vitals').then(({ onLCP, onFID, onINP, onCLS, onTTFB }) => {
        onLCP((metric) => { collectedMetrics.lcp = Math.round(metric.value); });
        onFID((metric) => { collectedMetrics.fid = Math.round(metric.value); });
        onINP((metric) => { collectedMetrics.inp = Math.round(metric.value); });
        onCLS((metric) => { collectedMetrics.cls = metric.value; }); // CLS is unitless, don't round
        onTTFB((metric) => { collectedMetrics.ttfb = Math.round(metric.value); });
    }).catch(() => {
